...
class RecurringTransaction(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    start_date = db.Column(db.DateTime(timezone=True), index=True, nullable=False)
    end_date = db.Column(db.DateTime(timezone=True), index=True) # leave null for perpetual
    amount = db.Column(db.Numeric, nullable=False)
    sign = db.Column(db.SmallInteger, nullable=False) # -1 for expense, 1 for income
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'))
    category_id = db.Column(db.Integer, db.ForeignKey('categories.id'))
    frequency = db.Column(db.Integer, nullable=False) # in hours

    user = db.relationship('User', backref=db.backref('recurring_transactions', lazy='dynamic'))
    category = db.relationship('Category', backref=db.backref('categories', lazy='dynamic'))

    def __init__(self, start_date, end_date=None, amount, sign, user_id, category_id, frequency):
    ...
    class Serializer(Serializer):
        class Meta:
            fields = ("id", "start_date", "end_date", "amount", "sign", "user_id", "category_id", "frequency")
